...
from Pyastra.lib.converters.tree2ol import AsmObject, Label
...
""" % (pyastra.lib.version, opts.get('infile', 'unknown'),
       proc, proc)
        ...
        for name in mem:
            ...
            if var.used and not var.special:
                addr = var.get_addr()
                self.data += f'{name}\tequ\t{hex(addr)}\t;bank {addr >> 7:g}\n'
                rmem += 1
        ...
        for ao in src:
            ...
            if opts['debug']:
                block += f'\t;bank {str(curr_bank)}\n'
                block += f'\t;pmem {hex(pmem)}\n'
                block += f'\t;org {hex(block_org)} + {hex(shift_org)} = {hex(block_org + shift_org)}\n'
            ao.addr_max = addr
            ao.finalize()
            ...
            if asm_object.pages[page][1] < shift_org + block_org:
                page += 1
                if page >= len(asm_object.pages):
                    opts['pyastra'].say('Program doesn\'t fit program memory!',
                                        pyastra.lib.ERROR)
                    return
                page_org = asm_object.pages[page][0]
                block_org = page_org
                block = ('\torg\t%s\n' % hex(block_org)) + block
